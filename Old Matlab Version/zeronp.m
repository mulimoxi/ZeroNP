...
function [p,jh,l,h,ic,count_cost]=zeronp(cnstr,op,l,h)
%% setting initiate point and box constrains
if exist('cnstr','var')<=0.5
  error('Syntax error')
end
%
om=['ZERONP--> ';'         '];  %error message
% 
np = 0;
Ipc = [0,0];
Ipb=[0 0]; 
...
if m<nic+1
  error([om(1,:) 'The number of constraints in your COST function does';...
        om(2,:) 'not match the number specified in the call to ZERONP.']);
end 
...
%-------------------------------------------------------------------------------
%
%The Function ZERONP solves nonlinear programs in standard form:
%
%        minimize              J(P)
%        subject to            EC(P)  =0
%                   IB(:,1)<=  IC(P)  <=IB(:,2)
